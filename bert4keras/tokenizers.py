...
class BasicTokenizer(object):
    ...
    def __init__(self, token_start='[CLS]', token_end='[SEP]'):
        """初始化
        添加了token_pad,unk,mask,start,end五种token。开始和结束用cls和sep。
        """
        self._token_pad = '[PAD]'
        self._token_unk = '[UNK]'
    ...
    def tokenize(self, text, maxlen=None):
        """分词函数
        """
        tokens = self._tokenize(text)       #使用基本分词函数分词，插入头和尾部，补全不足部分。
    ...
    def tokens_to_ids(self, tokens):
        """token序列转换为对应的id序列。对每个字进行编码，找到每个字的id。
        """
    ...
    def encode(
        ...
        #first_token_ids是token对应id号，segments_ids是0组成的list。
...
class Tokenizer(BasicTokenizer):
    ...
    def __init__(
        ...
        tokenize函数。
        """
        super(Tokenizer, self).__init__(**kwargs) #添加五种通用token。
        if is_string(token_dict): #如果token_dict是路径，就加载该token_dict.key是单词，value是对应id。
        ...
        for token in ['pad', 'unk', 'mask', 'start', 'end']:
            try:
                _token_id = token_dict[getattr(self, '_token_%s' % token)] #获取5中token词id。
                #设置self对象的_token_s_id属性值为_token_id,如果没有该属性，会给对象新增属性并赋值。
    ...
    def _tokenize(self, text, pre_tokenize=True):
        ...
        if self._do_lower_case:
            if is_py2:              #如果是py2就对它解码成二进制。
                text = unicode(text)
            text = text.lower()         #转换成小写。
            text = unicodedata.normalize('NFD', text)           #下面的看不懂，应该是一些格式转换的内容。
            text = ''.join([
