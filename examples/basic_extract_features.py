...
tokenizer = Tokenizer(dict_path, do_lower_case=True)  # 建立分词器，初始化了token_dict。对每个word都分配了id。
model = build_transformer_model(config_path, checkpoint_path)  # 建立模型，加载权重

# 编码测试
token_ids, segment_ids = tokenizer.encode(u'语言模型') #会加上cls头部和sep尾部，输出6个id。
token_ids, segment_ids = to_array([token_ids], [segment_ids]) #对每个字编码得到对应id，seg_id为0。
